...
vAngDeg = [np.float32(i) for i in range(int(-NFFTAnt / 2), int(NFFTAnt / 2))];
...
plt.plot(AzSearch, (Zmax - np.log10(Z))/(Zmax - Zmin),label='music plot');#music plot
...
plt.plot(vAngDeg, -(JOpt_min - np.log10(np.sum(np.abs(JOpt), 1)))/(JOpt_max - JOpt_min),label='dbf plot');#DBF plot   
plt.ioff();
plt.legend()
plt.show();
